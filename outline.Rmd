...
If you want to make a histogram, `hist` is probably a good option.
...
For our example, we'll  use the 2nd data.frame from our project folder, which already has model chlorophyll as a function of TP. The units are log-transform though, so we'll untransform using `exp`.
...
##Customizing plots
You may have noticed that in `plot` help file, there are a number of arguments, including  `xlab` and `ylab` to name your axes and `main` for the plot title. These arguments require text strings: 
...
Then, in the help files for `plot`,`lines`, etc., there is something called "Further graphical parameters (see par)...". `par` is essentially a set of default graphical parameters.   Use `?par` to see the numerous options. We won't walk through all of them now, but it is worth exploring these options if you want to fully customize your plots. Many of them can be changed within the `plot` command. 

  some table here with log, col, pch, cex, lty, lwd
...
## Customizing plots
Now, let's finally try to get rid of that useless white space at the top of our plot. To do that, we'll introduce some new functionality of `par` settings. 

There are some arguments that can only be set in a specific call to the `par` function.  `mar` takes a vector of 4 values, where each is the number of lines of space provided on each side of the plot in the form c(bottom, left, top, right). The default is c(5.1,4.1,4.1,2). So, R is provided 4.1 lines of space above the plot (the 3rd value in that vector). To reduce that to something small, we just reset its value in par: `par(mar=c(5.1,4.1,0.5,2))`. 
